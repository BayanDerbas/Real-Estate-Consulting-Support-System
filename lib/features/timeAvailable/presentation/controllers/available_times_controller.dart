...
import '../../data/models/delete_time_model.dart';
...
class AvailableTimesController extends GetxController {
  ...
  Future<void> deleteTime(int id) async {
    isLoading.value = true;
    final Either<Failures, DeleteTimeModel> result = await repository.delete_time(id);

    result.fold(
          (failure) => Get.snackbar('Error', failure.err_message),
          (response) {
        Get.snackbar('Success', 'Time deleted successfully');
        availableTimes.removeWhere((time) => time.id == id);
      },
    );
    isLoading.value = false;
  }
