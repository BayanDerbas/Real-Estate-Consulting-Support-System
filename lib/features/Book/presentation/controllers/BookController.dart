...
class BookController extends GetxController {
  ...
  final availableTimes = <WorkingTimesData>[].obs;   // كل الأوقات المسترجعة من السيرفر
  ...
  List<WorkingTimesData> _filterTimesForSelectedDate() {
    ...
    return availableTimes.where((slot) {
      try {
        final slotWeekday = _mapDayOfWeek(slot.dayOfWeek ?? '');
        return slotWeekday == weekday;
      } catch (_) {
        return false;
      }
    }).toList();
  ...
  List<String> generateAppointmentHours() {
    ...
    if (todaysSlots.isEmpty) {
      print("لا توجد أوقات عمل لليوم ${selectedDate.value}");
      return [];
    }
    ...
    print("الساعات المولدة بعد الفلترة: $sortedSlots");
    return sortedSlots;
  }
