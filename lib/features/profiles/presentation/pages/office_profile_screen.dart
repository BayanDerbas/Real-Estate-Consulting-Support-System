...
class OfficeProfileScreen extends StatelessWidget {
  final OfficeProfilingController controller =
      Get.find<OfficeProfilingController>();

  OfficeProfileScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => controller.fetchOfficeByRole(),
      ...
      body: Obx(() {
        ...
        final profile = controller.officeProfile.value?.data;
        if (profile == null) {
          return Center(
            child: Text(
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildInfoCard(
                "block counts",
                "${profile.user?.blocksCount ?? 0} Followers",
              ),
              _buildInfoCard(
                "warns count",
                "${profile.user?.warnsCount ?? 0} Favorites",
              ),
            ],
          ),
        );
